...
@section('content')
    ...
    <form action="/admin/admin/" method="post" class="layui-form">
        @csrf
        ...
        <div class="layui-form-item container-fluid">
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="text" name="nickname" value="{{old('nickname')}}" placeholder="请输入昵称"
                           class="form-control" autocomplete="off">
                </div>
                <label class="col-sm-5 col-form-label text-muted pl-0">
                    长度2-20位
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6" style="line-height: 35px;">
                    <div class="custom-control custom-radio custom-control-inline" style="line-height: 24px;">
                        <input lay-ignore v-model="access_type" type="radio" name="access_type" value="0" @if(!old('access_type')) checked
                               @endif class="custom-control-input" id="access_type_0">
                        <label class="custom-control-label" for="access_type_0">角色权限</label>
                    </div>
                    <div class="custom-control custom-radio custom-control-inline" style="line-height: 24px;">
                        <input lay-ignore v-model="access_type" type="radio" name="access_type" value="1" @if(old('access_type')) checked
                    ...
                    <div class="row">
                        <div class="col-sm-12" style="height: 32px;line-height: 32px;">
                            <div class="custom-control custom-checkbox" style="line-height: 24px;">
                                <input lay-ignore v-model="supperadmin" type="checkbox" name="bs_role_ids[]" value="{{$roles[0]['id']}}"
                                       @if(old('bs_role_ids') && in_array($roles[0]['id'],old('bs_role_ids'))) checked
                                       @endif
                                ...
                                <div class="col-sm-3" style="height: 28px;line-height: 28px;">
                                    <div class="custom-control custom-checkbox" style="line-height: 24px;">
                                        <input lay-ignore type="checkbox" name="bs_role_ids[]" value="{{$v['id']}}"
                                               @if(old('bs_role_ids') && in_array($v['id'],old('bs_role_ids'))) checked
                                               @endif
    ...
    <script>
        require(['vue','jquery'], function (Vue,$) {
            new Vue({
                el:"#app",
                data:{
                    access_type: @if(old('access_type')=='1') 1 @else 0 @endif,
                    supperadmin: @if(old('bs_role_ids') && in_array($roles[0]['id'],old('bs_role_ids'))) true @else false @endif
                }
            });
